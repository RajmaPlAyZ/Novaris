...
export const Item = ({
  ...
  return (
    <div
      onClick={onClick}
      role="button"
      style={{ paddingLeft: level ? `${level * 12 + 12}px` : "10px" }}
      className={cn(
        "group text-muted-foreground hover:bg-primary/8 flex min-h-7.5 w-full items-center rounded-md py-1.25 pr-2.5 text-sm font-medium transition-colors",
    ...
    >
      ...
      {shortcut && (
        <kbd className="bg-muted text-muted-foreground pointer-events-none ml-auto inline-flex h-5 items-center gap-1 rounded border px-1.5 font-mono text-[.625rem] font-medium opacity-100 select-none">
          {shortcut}
        </kbd>
      )}
      {!!id && (
